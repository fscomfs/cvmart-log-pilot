...
crosscompile: ## @build Cross-compile beat for the OS'es specified in GOX_OS variable. The binaries are placed in the build/bin directory.
	...
	GOOS=linux GOARCH=amd64 go build -o  ${BUILD_DIR}/bin/cvmart-log-pilot-linux-amd64
	GOOS=linux GOARCH=arm64 CC=aarch64-linux-gnu-gcc go build -o ${BUILD_DIR}/bin/cvmart-log-pilot-linux-arm64
#
#
